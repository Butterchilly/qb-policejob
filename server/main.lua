...
RegisterNetEvent('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    if #(playerCoords - targetCoords) > 5.5 then return DropPlayer(src, "Attempted exploit abuse") end
    ...
    if CuffedPlayer.PlayerData.metadata.ishandcuffed then 
        TriggerClientEvent("police:client:cuffanimation", Player.PlayerData.source, "uncuff")
    else
        TriggerClientEvent("police:client:cuffanimation", Player.PlayerData.source, "cuff")
    end  
end)
...
RegisterNetEvent('police:server:showFingerprintId', function(sessionId)
    ...
    local player = QBCore.Functions.GetPlayer(src)
    TriggerClientEvent('chat:addMessage', sessionId, {
        color = { 0, 0, 255},
        multiline = true,
        args = {'Police Records', "Found record for ("..src..") CSN: "..player.PlayerData.citizenid}
    })
end)
