...
local function UnCuffAnimation()
    local ped = PlayerPedId()
...
-- Events
RegisterNetEvent('police:client:cuffanimation', function(value)
    if value == "cuff" then
        HandCuffAnimation()
    elseif value == "uncuff" then
        UnCuffAnimation()
    end
end)
...
RegisterNetEvent('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if IsEntityPlayingAnim(playerPed, "missminuteman_1ig_2", "handsup_base", 3) or IsEntityPlayingAnim(playerPed, "mp_arresting", "idle", 3) or IsTargetDead(playerId) then
            QBCore.Functions.Progressbar("robbing_player", Lang:t("progressbar.robbing"), math.random(10000, 14000), false, true, {
